...
def print_data(data):
    ...
    if 'status' in data:
        status = ByteToHex( data['status'])
        print '\nStatus: ', status
    
    if 'frame_id' in data:
        frame_id = data['frame_id']
        print 'Frame id: ',frame_id ,'/', ByteToHex( data['frame_id'])
        
    if 'source_addr_long' in data:
        source_addr_long = ByteToHex( data['source_addr_long'])
    ...
    if 'samples' in data:
        if 'adc-7' in data['samples'][0]:
            ...
            print 'Testig', ByteToHex( str(data['samples'][0]['adc-7']))
    ...
    if 'parameter' in data:
        if 'status' in data['parameter']:
            status = ByteToHex( data['parameter']['status'])
            print 'Status: ', status
        
        if 'source_addr' in data['parameter']:
            source_addr = ByteToHex( data['parameter']['source_addr'])
            print 'source_addr: ', source_addr
        
        if 'parent_address' in data['parameter']:
            parent_address = ByteToHex( data['parameter']['parent_address'])
            print 'parent_address: ', parent_address
        
        if 'profile_id' in data['parameter']:
            profile_id = ByteToHex( data['parameter']['profile_id'])
            print 'profile_id: ', profile_id
        
        if 'source_addr_long' in data['parameter']:
            source_addr_long = ByteToHex( data['parameter']['source_addr_long'])
            print 'source_addr_long: ', source_addr_long
        
        if 'device_type' in data['parameter']:
            device_type = ByteToHex( data['parameter']['device_type'])
...
MY = HexToByte('0B41')
ADR = HexToByte('0013A2004092D86A')
